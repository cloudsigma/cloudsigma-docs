...
Resources requested by user are usually allocated to maximize performance. However this can lead to a situation, where
...
To improve the robustness of redundant setups, it is possible to hint the system, which resources are preferred to be
...
Returns which running servers share same physical computes host. Returns an array containing arrays.
...
Queries which other servers share same physical host with the given one. Returns an array holding server UUIDs. The
...
Returns which drives share same physical storage host. Returns an array containing arrays.
Each inner array holds the UUIDs of drives which reside on same physical host.

.. http:get:: /drives/availability_groups/{uuid}/

:statuscode 200: no error

Queries which other drives share same physical storage host with the given one. Returns an array holding drives UUIDs.
...
Avoid functionality is **best effort**. This means that requests containing avoid will succeed even if the avoid can not
...
Avoid functionality may incur **performance penalty**. Specifying avoid for drives cloning and servers cloning, as it
...
Starts a server with specific UUID attempting to run it on a different physical infrastructure host from the other
...
It is possible to hint the system which drives are preferred to be on separate physical storage hosts. Avoid can be
...
Note that it might not always be create a drive in a different availability group, therefore the order of the
...
The best way to create a clone of server that does not share hardware with the original is to clone the origin server
with avoiding itself, and to start the clone with avoiding the origing:
